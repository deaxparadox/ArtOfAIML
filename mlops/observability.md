...
Kubernetes' probes check whether a container is alive and ready — a real but narrow signal, and [Kubernetes](kubernetes.md#common-mistakes)'s own point stands: a probe checking nothing meaningful defeats its purpose. A service can pass every health check and still behave badly in ways a simple probe was never built to catch — requests taking far longer than they should, a model's predictions quietly drifting the way [ML Workflow](../machine-learning/ml-workflow.md) already warned about (see [Drift Monitoring](drift-monitoring.md) for that specific, slower kind of problem), or one specific type of request silently failing more often than others. Observability exists to make that deeper behavior visible and queryable after the fact, rather than something a person has to have predicted and built a specific check for in advance.
